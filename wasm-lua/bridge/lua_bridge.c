...
static const char *BOOTSTRAP_LUA =
    /* ---- print (uses C bridge – safe inside C callbacks like gsub) ---- */
    "local _bw = __bridge_write\n"
    "print = function(...)\n"
    "  local parts = {}\n"
    "  for i = 1, select('#', ...) do\n"
    ...
    "  end\n"
    "  parts[#parts+1] = '\\n'\n"
    "  _bw(table.concat(parts))\n"
    "end\n"
    /* ---- io.write (uses C bridge – safe inside C callbacks) ---- */
    "io.write = function(...)\n"
    "  local parts = {}\n"
    "  for i = 1, select('#', ...) do\n"
    "    parts[#parts+1] = tostring(select(i, ...))\n"
    ...
    /* ---- io.open (virtual file handle – write-only, triggers download on close) ---- */
    "do\n"
    "  local _real_open = io.open\n"
    "  io.open = function(path, mode)\n"
    "    mode = mode or 'r'\n"
    "    if mode:find('r') then\n"
    "      return nil, 'io.open: reading files is not supported in the browser'\n"
    "    end\n"
    "    if not (mode == 'w' or mode == 'wb' or mode == 'a' or mode == 'ab') then\n"
    "      return nil, 'io.open: unsupported mode: ' .. mode\n"
    "    end\n"
    "    local buf = {}\n"
    "    local closed = false\n"
    "    local fh = {}\n"
    "    function fh:write(...)\n"
    "      if closed then error('attempt to write to a closed file', 2) end\n"
    "      for i = 1, select('#', ...) do\n"
    ...
    "      closed = true\n"
    "      coroutine.yield('__file_save', path, table.concat(buf))\n"
    "    end\n"
    "    function fh:flush() end\n"
    "    function fh:lines() error('lines() not supported in browser', 2) end\n"
    "    function fh:read() error('read() not supported on write handle', 2) end\n"
    "    function fh:seek() error('seek() not supported in browser', 2) end\n"
    "    return fh\n"
    "  end\n"
    "end\n"
